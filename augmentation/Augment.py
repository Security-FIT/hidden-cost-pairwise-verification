...
class Augmentor:
    ...
    def __init__(self, rir_root):
        ...
        # The RIRDataset object initializes a dataframe from the RIR protocols. These are not available locally
        # so it crashes on FileNotFoundError, meaning it disrupts local prototyping. 
        # Uncomment the following line if you use RIR augmentation and have the RIR dataset (and protocols)
        # available
    ...
    def augment(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():  # No need to compute gradients for augmentation
            ...
            # if apply_rir:
            #     rir_intensity = random.uniform(0.2, 0.8)
            #     waveform = self.RIR.apply_rir(waveform, scale_factor=rir_intensity)
                # print(f"Applied RIR with intensity {rir_intensity}")
