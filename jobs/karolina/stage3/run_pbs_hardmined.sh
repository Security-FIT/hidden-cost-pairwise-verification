#!/bin/bash
#
# Helper script to submit MLAAD Stage 3 hardmined training jobs with different seeds.
#
...
JOBS=(
  # "MLAADCurriculumHardminedFFCosineDataset_pair|MHFA|FFCosine|30:00:00|/scratch/project/dd-25-3/DFS-Detection-Framework/runs/karolina_stage2_intermediate_MHFA_FFCosine_seed42_20251204_172405/FFCosine_25.pt"
  "MLAADCurriculumHardminedFFCosineDataset_pair_s123|MHFA|FFCosine|30:00:00|/scratch/project/dd-25-3/DFS-Detection-Framework/runs/karolina_stage2_intermediate_MHFA_FFCosine_seed123_20251204_172405/FFCosine_10.pt|11|123"
  "MLAADCurriculumHardminedFFCosineDataset_pair_s222|MHFA|FFCosine|30:00:00|/scratch/project/dd-25-3/DFS-Detection-Framework/runs/karolina_stage2_intermediate_MHFA_FFCosine_seed222_20251204_172404/FFCosine_30.pt|31|222"
)
...
for ENTRY in "${JOBS[@]}"; do
  ...
  JOB_SCRIPT="${SCRIPT_DIR}/train_seed_hardmined_${PROCESSOR}_${CLASSIFIER}_${SEED}.sh"
  sed -e "s/{{SEED}}/${SEED}/g" \
      -e "s/{{DATASET}}/${DATASET}/g" \
      -e "s/{{WALLTIME}}/${WALLTIME}/g" \
      -e "s/{{PROCESSOR}}/${PROCESSOR}/g" \
      -e "s/{{CLASSIFIER}}/${CLASSIFIER}/g" \
      -e "s/{{MODE}}/curriculum_hardmined/g" \
      "${TEMPLATE}" > "${JOB_SCRIPT}"
  chmod +x "${JOB_SCRIPT}"
  XLSR_MODE="frozen"
  ...
  echo "Submitting Stage 3 hardmined on Karolina (${PROCESSOR}+${CLASSIFIER}) with seed ${SEED} (walltime ${WALLTIME}; XLSR ${XLSR_MODE})..."
  if [[ -n "${CHECKPOINT}" ]]; then
    ...
    EXPORT_VARS="ALL,CHECKPOINT=${CHECKPOINT},NUM_EPOCHS=1,START_EPOCH=${START_EPOCH},LR_EPOCH_MULTS=1:0.01,RUN_EVAL_AFTER_TRAIN=1,FINETUNE_SSL=${FINETUNE_SSL}"
    if [[ -n "${EXTRACTOR_LR}" ]]; then
      EXPORT_VARS="${EXPORT_VARS},EXTRACTOR_LR=${EXTRACTOR_LR}"
    fi
    sbatch --export="${EXPORT_VARS}" "${JOB_SCRIPT}"
  else
    EXPORT_VARS="ALL,NUM_EPOCHS=1,START_EPOCH=26,LR_EPOCH_MULTS=1:0.01,RUN_EVAL_AFTER_TRAIN=1,FINETUNE_SSL=${FINETUNE_SSL}"
    if [[ -n "${EXTRACTOR_LR}" ]]; then
      EXPORT_VARS="${EXPORT_VARS},EXTRACTOR_LR=${EXTRACTOR_LR}"
    fi
    sbatch --export="${EXPORT_VARS}" "${JOB_SCRIPT}"
