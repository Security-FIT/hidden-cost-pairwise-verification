#!/bin/bash
#SBATCH --job-name=mlaad_ff_embbneck10_s222
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/karolina_baseline_ff_multiclass_embbneck10_s222_${RUN_TIMESTAMP}.train.log"
...
srun -u python -u train_and_eval.py \
  ...
  ${AMP_EVAL_FLAG} \
  "${AMP_DTYPE_FLAG[@]}" \
  --output_dir runs/karolina_baseline_ff_multiclass_embbneck10_s222_${RUN_TIMESTAMP} \
